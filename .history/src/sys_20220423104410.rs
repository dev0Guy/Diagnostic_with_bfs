...
pub struct System{
    id: String,
    id_to_gate_option: Vec<(String, GateInfo)>,
}
...
impl System{
    ...
    pub fn activate(&self, input: &OBS) -> bool{
        for (key,info) in &self.id_to_gate_option{
            println!("({},{:?})",key,info);
        }
        false
    }
    ...
    pub fn new(path: &str) -> System{
        ...
        let _ = content.pop().unwrap();
        let mut id_to_gate_option: Vec<(String, GateInfo)> = Vec::new();
        for line in content.iter() {
            ...
            let inputs: Vec<String> = tokens.iter().rev().map(|&s|s.into()).filter(|x| x!="").collect();
            let gate_info = GateInfo(gate,inputs,output);
            id_to_gate_option.insert(gate_id, gate_info);
        }
        System{id,id_to_gate_option}
    }  
