...
#[derive(Debug)]
struct GateInfo(String,Vec<String>,String);

#[derive(Debug)]
pub struct System{
    id: String,
    id_to_gate_option: HashMap<String,GateOptions>,
...
impl System{
    ...
    pub fn new(path: &str) -> (){
        ...
        let _ = content.pop().unwrap();
        let _ = content.pop().unwrap();
        let mut id_to_gate_option: HashMap<&str, GateOptions> = HashMap::new();
        for line in content.iter() {
            ...
            println!("{:?}",tokens);
            let id = tokens[1];
            id_to_gate_option.insert(id, gate);
        }
